<xsl:stylesheet xmlns:s="http://www.w3.org/2001/sw/DataAccess/rf1/result" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	<xsl:template match="/">
		...
		<div id="icg_content">
					...
					<xsl:for-each select="/s:sparql/s:results/s:result">
						<tr>
							...
							<td class="col_200">
								...
								<xsl:variable name="myTermUrl">
									<xsl:value-of select="substring-before($myTerm,'#')"/>
									<xsl:text>%23</xsl:text>
									<xsl:value-of select="substring-after($myTerm,'#')"/>
								</xsl:variable>
								...
								<xsl:choose>
									<xsl:when test="s:k1 > 0">
										<!-- make urls here -->
										<a
											href="../includes/icg_make_query.php?icg_stype=member_inc&amp;icg_sterm={$myTermUrl}&amp;icg_starget=info:fedora/{$pid}&amp;icg_server={$myServer}&amp;submit=Submit">
			...
			<div id="icg_footer">
				<p>XSLT: icg_is_member_of.xsl<br/>CSS: icg_results.css</p>
			</div>
