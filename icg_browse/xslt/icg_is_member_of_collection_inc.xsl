<xsl:stylesheet xmlns:s="http://www.w3.org/2001/sw/DataAccess/rf1/result" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/">
		...
		<div id="icg_content">
			...
			<table id="icg_table" class="tablesorter" cellspacing="1">
				...
				<tbody>
					<xsl:for-each select="/s:sparql/s:results/s:result">
						...
						<xsl:choose>
							<xsl:when test="contains(s:relation/@uri,'isMemberOfCollection') and s:target/@uri = $myTarget">
								<tr>
									<td class="col_400">
										...
										<a>
											...
											<xsl:attribute name="title">View Data Object</xsl:attribute>
											<xsl:value-of select="substring-after(s:object/@uri, '/')"/>
										</a><xsl:text> - </xsl:text>
										<a
											href="../includes/icg_make_query.php?icg_stype=collection2_inc&amp;icg_sterm=info:fedora/fedora-system:def/relations-external%23isMemberOf&amp;icg_starget=info:fedora/{$pid}&amp;icg_server={$myServer}&amp;submit=Submit">
											<xsl:attribute name="title"><xsl:text>View constitutent parts of </xsl:text><xsl:value-of select="$pid"/></xsl:attribute>(Related Parts)</a>
										<br/>
			...
			<div id="icg_footer">
				<p>XSLT: icg_is_member_of_collection_inc.xsl<br/> CSS: icg_results.css</p>
			</div>
