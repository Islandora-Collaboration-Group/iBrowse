<xsl:stylesheet xmlns:s="http://www.w3.org/2001/sw/DataAccess/rf1/result" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/">
		...
		<div id="icg_content">
			...
			<table id="icg_table" class="tablesorter" cellspacing="1">
				...
				<tbody>
					<xsl:for-each select="/s:sparql/s:results/s:result">
						...
						<xsl:choose>
							<xsl:when test="contains(s:relation/@uri,'isMemberOf') and not(contains(s:relation/@uri,'isMemberOfCollection'))">
								<tr>
									<td class="col_400">
			...
			<div id="icg_footer">
				<p>XSLT: icg_is_member_of_inc.xsl<br/> CSS: icg_results.css</p>
			</div>
